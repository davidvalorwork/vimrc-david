...
function saveDeck() {
  if (__currentDeck) {
    localStorage.setItem(`deck-${__name}`, __currentDeck);
  }
}
...
lib.openDeck = function (name, minDiff, maxDiff) {
  ...
  if (localStorage.getItem(`deck-${name}`) === null || localStorage.getItem(`deck-${name}`) === undefined) {
    localStorage.setItem(`deck-${name}`, new Deck(name));
  }
  __currentDeck = localStorage.getItem(`deck-${name}`);
  __name = name;
  reset();
...
lib.listDecks = function () {
  ...
  for (let i = 0; i < len; i += 1) {
    ...
    const obj = {};
    if (name) {
      const parsed = localStorage.getItem(name[0]);
      [, obj.name] = name;
...
export default lib; // eslint-disable-line no-param-reassign